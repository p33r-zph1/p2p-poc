...
function fieldLengthError(field: string, length: number, exact = true) {
  return `Field ${field} must be ${length} characters ${
    exact ? '' : 'or less '
  }long.`;
}

export function getErrors(qrData: Record<string, any>) {
  const { countryCode, city, name, accountNumber, swiftCode, bankName } =
    qrData;
  ...
  if (countryCode?.length !== 2)
    emvCoErrors.push(fieldLengthError('country code', 2));
  ...
  if (swiftCode?.length !== 11)
    customErrors.push(fieldLengthError('swift code', 11));
  if (!swiftCode && !bankName)
    customErrors.push(requiredFieldError('bank name'));
  if (swiftCode && !bankName)
    customErrors.push('Bank is not yet added to the list.');
